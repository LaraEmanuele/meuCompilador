...
bool verificar_ll1(Grammar *g) {
    ...
    // Uso dos teus iteradores nativos
    Production **regras = productions_for(g, nt_s);
    
    const char *terminais_vistos[10];
    int qtd_vistos = 0;
    bool tem_conflito = false;

    for (int i = 0; regras[i] != NULL; i++) {
        Production *p = regras[i]; // <--- CORRIGIDO AQUI! Linha limpa.
        ...
        if (p->rhs_count == 0 || sequencia_rhs == NULL) {
            ...
            // Pega o primeiro terminal da direita
            predict_symbol_name = sequencia_rhs[0]->name;
        }

        // Print visual para o professor ver a mágica a acontecer
        ...
        // Validação Matemática de Interseção de Conjuntos (Proibido duplicados)
        for (int v = 0; v < qtd_vistos; v++) {
            if (strcmp(terminais_vistos[v], predict_symbol_name) == 0) {
                tem_conflito = true;
                printf("[CONFLITO LL(1) DETECTADO]: O simbolo '%s' quebra a predicao!\n", predict_symbol_name);
            }
        }
        ...
        if (sequencia_rhs != NULL) {
            free(sequencia_rhs); // Limpa a esteira gerada por rhs()
        }
    }

    free(regras); // Limpa a esteira gerada por productions_for()
